...
class SoatBase(SQLModel):
    ...
    def validate_type(cls, v: str) -> str:
        valid_types = ['CAR', 'MOTORCYCLE', 'TRUCK', 'BUS', 'SPECIAL']  # Ejemplo de tipos
...
class Soat(SoatBase, table=True):
    ...
    vehicle: "Vehicle" = Relationship(back_populates="technomechanics")
    driver_data: Optional["DriverData"] = Relationship(back_populates="soat")

class SoatCreate(SoatBase):
    pass

class SoatRead(SQLModel):
    id: int
    vehicle_id: int
    expiration_date: datetime
    registration: str
